...
### ~~남은 결정 2건~~ → **둘 다 해소** (Phase 6에서 구현 완료)

1. ~~**알바생이 자기 과거 근무·급여를 조회할 수 있어야 하는가?**~~
   → **읽기 허용 · 쓰기 차단.** 근무 등록·출퇴근·수정·삭제와 급여 설정 변경은 막고,
   조회는 허용한다. 보존의 목적이 알바생의 임금·소득 증빙인데 정작 당사자가 못 보면
   보존의 의미가 없다. 쓰기를 막는 이유는 승인할 사장님이 없어 변경이 무의미하기 때문이다.
   **근무지 탈퇴는 막지 않는다** — 막으면 알바생이 사라진 근무지에 영원히 묶인다.
   구현: `PermissionVerifyUtil.verifyWorkplaceIsWritable` (`abf48ee`)
2. ~~**소프트 삭제 유예 3일 동안의 동작.**~~
   → **이미 확정돼 있었다.** [확정 정책 16](README.md#확정-정책-색인)(스코프 4 Q2) —
   탈퇴 유예기간 중에는 재로그인 전까지 전면 차단이며 `f5bb991`로 구현 완료다.
   이 문서가 그 답변을 반영하지 못한 채 남아 있었다.
...
| 항목 | 승인 대기 중 | 근거 |
|---|---|---|
| (a) 근무지 이름·주소 | **허용** | 어디에 참여 신청했는지 확인 가능해야 함 |
| (b) 자기 급여 설정 | **허용** | 참여 시 본인이 입력한 값 |
| (c) 근무 등록·출퇴근 | **차단** | 승인 전에는 실제 근무 행위 불가 |
| (d) 근무지 목록 표시 | **허용 — 단 `PENDING_APPROVAL` 상태로 표시** | (a)로 가는 진입점 확보 |

(d)를 허용하는 이유: 목록에서 숨기면 앱을 껐다 켠 대기자가 **자신이 어디에 승인 대기
중인지조차 알 수 없다.** 목록에는 띄우되 상태로 구분한다.

### 상태 필드 설계

**`WorkplaceSummaryResponse`** (알바생 시점) — `status` 열거형 추가:

| 값 | 의미 | 프론트 |
|---|---|---|
| `ACTIVE` | 정상 | 평소대로 |
| `PENDING_APPROVAL` | 승인 대기 | 배지/별도 섹션, 근무 등록 진입 차단 |
| `OWNER_WITHDRAWN` | 사장님 탈퇴 (확정 정책 5) | 미표시 또는 별도 표시 |

열거형 하나로 확정 정책 5(사장님 탈퇴)와 6(승인 대기)의 표시 요구를 함께 만족시킨다.

**`WorkerSummaryResponse`** (사장님 시점) — 승인 여부 필드 추가.
현재 필드가 `workerId`, 라벨색 2, `nickname`, `profileImg`뿐이라 **사장님이 누가
대기자인지 알 수 없고 승인 버튼을 어디에 띄울지 판단할 수 없다.**

### 서버 측 차단 지점

(c)만 차단하면 되고, 지점은 C-1과 동일하다 — `PermissionVerifyUtil`에서
`is_accepted`를 검사하되 **(a)(b)(d)를 담당하는 읽기 경로는 예외로 둔다.**
즉 "모든 경로 차단 후 화이트리스트"가 아니라 "근무 행위 경로만 차단"이다:

- 차단: `WorkService`의 근무 생성·수정·삭제·출퇴근 경로
- 허용: `WorkplaceService.getWorkplaceDetail`, `getWorkplace`, `getAllWorkplace`
...
| # | 질문 | 블로킹 |
|---|---|---|
| ~~**Q1**~~ | **답변 완료 → (a)(b)(d) 허용, (c) 차단. (d)는 목록에 `PENDING_APPROVAL` 상태로 표시.** 아래 "확정 정책 6" 참조 | 해소 |
