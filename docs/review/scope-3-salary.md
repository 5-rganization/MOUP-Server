...
| # | 질문 | 블로킹 |
|---|---|---|
| **Q1** | 시급 인상 시 **이미 등록된 미래 예정 근무**의 급여는 새 시급으로 재계산되어야 하는가, 등록 시점 스냅샷을 유지해야 하는가? **과거 근무**는? (`works.hourly_rate` 스냅샷 컬럼의 존재는 "유지"를 시사하는데, 그렇다면 `recalculateEstimatedNetIncomeForMonth` 호출 자체가 모순) | **C-3, C-6 수정 방향** |
