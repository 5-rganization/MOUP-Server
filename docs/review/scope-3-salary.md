...
| **Q9** | (원장 추가) 주휴수당을 법정 산식 `(주 소정근로시간/40) × 8 × 시급`(40h 상한)으로 확정하는가? | **C-1 수정 방향** |
| **Q10** | (Q1 후속) 주 중간에 시급이 바뀌어 한 주 안에 **스냅샷 시급이 섞이면** 주휴수당 기준 시급은 무엇인가? (가중평균 / 그 주 첫 근무 / 그 주 마지막 근무) 현재는 `weekWorks.get(0)`이라 사실상 "가장 이른 근무"가 임의 선택된다 | **I-2 수정 방향** |
| **Q11** | (Q1 후속) 스냅샷 정책에서 **시급을 잘못 입력한 경우 정정 경로**가 필요한가? 현재는 이미 등록된 근무에 반영할 방법이 없어, 오타를 고치려면 미래 근무를 전부 삭제·재등록해야 한다. "이후 근무에 일괄 적용" 같은 명시적 액션이 필요할 수 있다 | — |
