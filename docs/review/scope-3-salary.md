...
한 주 안에 스냅샷 시급이 섞일 때의 주휴수당 기준 시급은 **근무 시간으로 가중한
평균**으로 한다. `weekWorks.get(0)`(= `ORDER BY work_date`의 가장 이른 근무,
`WorkRepository.java:114`)은 선택이 아니라 우연이므로 폐기한다.

**선행 조건 3건 — 이것들을 먼저 고치지 않으면 가중평균이 현재보다 나빠진다:**

| 선행 | 이유 |
|---|---|
| **I-2** (`hourly_rate` NULL 처리) | 현재 `get(0)`은 NULL 노출이 1/n이지만, **가중평균은 모든 근무를 보므로 NULL 한 건이 항상 전체 평균을 끌어내린다.** null을 0으로 채우면 그 주 주휴수당이 통째로 낮아진다 → 0 대신 해당 근무를 가중에서 **제외**할 것 |
| **M-8** (음수 net 클램프) | net이 음수인 근무는 **가중치가 음수**가 되어 평균이 왜곡된다. `Math.max(0, ...)` 선행 필수 |
| **C-2** (payableWorks 통일) | 가중평균도 반드시 `end_time != null` 필터된 **같은 리스트**에서 계산해야 한다. 아니면 집합 불일치가 하나 더 생긴다 |

**가중치는 `netWorkMinutes` 기준**으로 한다 — 주 근로시간 판정(15h/40h)이 net
기준이므로 통일한다.

**잔여 리스크 (노무 확인 대상)**: 주휴수당의 통상임금 기준을 **"주휴일이 속한
시점"** 으로 본다면 정답은 가중평균이 아니라 **그 주 마지막 시급**이다. 시급이
인상된 주에서 가중평균은 인상 후 시급보다 낮으므로 **근로자에게 불리한 방향**이다.
Q9의 노무 확인 시 이 항목도 함께 물을 것.

**부수 결정 필요**: 가중평균은 알바생에게 설명이 어렵다("왜 이 금액이지?").
주휴수당 표시 시 기준 시급(예: "평균 시급 11,015원 기준")을 함께 노출할지
UI 결정이 필요하다.
...
| ~~**Q10**~~ | **답변 완료 → 가중평균** (근무 시간으로 가중). 선행 조건 3건과 잔여 리스크는 아래 "확정 정책 4" 참조 | 해소(조건부) |
