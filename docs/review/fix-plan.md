...
### 계획 정정 — "DDL이 필요 없다"가 틀렸다

계획서는 `owner_id IS NULL`(FK의 `ON DELETE SET NULL`)을 "사장님 탈퇴" 신호로 쓰면
되므로 DDL이 불필요하다고 봤다. **그 전제가 [확정 정책 7](scope-5-workplace.md#확정-정책-7--가명처리-구현-명세-q2-답변)과
모순된다.** 정책 7은 하드 삭제를 없애고 가명처리하라는 것인데, 그러면 `owner_id`가
애초에 NULL이 되지 않는다.

`users.anonymized_at` 컬럼 하나가 필요했다. 이유는 아래 3번.

### 한 일

`DELETE FROM users`가 문제였다. `workplaces.owner_id`가 SET NULL이 되면서 사장님이
만든 근무지에 남아 있던 **알바생들의 근무·급여 이력까지 함께 무너졌다.** 그 데이터는
사장님만의 것이 아니라 알바생의 임금·소득 증빙이기도 하다.

행을 남기고 개인정보만 제거한다. 하드 삭제를 없앤 결과 `users`의 CASCADE가 더 이상
발화하지 않으므로 지워야 할 것을 직접 지운다 — `social_tokens` · `user_tokens` ·
`fcm_tokens` · `routines` · `normal_alarms` · `admin_alarm_user_mappings`.
하나라도 빠뜨리면 탈퇴자의 소셜 자격증명이 DB에 그대로 남는데 **아무 증상이 없다.**
S3 프로필 이미지 삭제도 함께 넣었다(예전 하드 삭제도 이걸 지우지 않아 객체가 남았다).

> 정책 7 명세의 `fcm_token = NULL`은 Phase 4에서 `fcm_tokens` 테이블로 분리되면서
> 컬럼 자체가 사라졌다. `DELETE FROM fcm_tokens`로 대체했다.

### 구현하며 드러난 것 — 전부 "조용히 깨지는" 것들이다

1. **배치가 같은 사용자를 무한 재처리하게 된다.**
   행이 남으므로 `is_deleted = 1`이 영구히 참이다. 탈퇴 배치 조회 3개가 이미 처리한
   사용자를 매일 다시 집어 소셜 연동 해제를 반복한다. 로그를 보지 않으면 알 수 없다.
   → `users.anonymized_at`으로 "신청됨"과 "처리 완료"를 구분한다.

2. **`OWNER_WITHDRAWN`이 영영 뜨지 않게 된다.**
   `resolveStatus`가 `owner_id IS NULL`로 판정하는데 가명처리하면 그 값이 살아남는다.
   알바생은 사장님이 사라진 것을 알 수 없다.
   → 소유자의 `is_deleted`를 함께 본다. 목록 조회는 N+1을 피해 배치 쿼리 1회.

3. **탈퇴한 알바생에게 근무를 배정할 수 있게 된다** ([스코프 5 함정 2](scope-5-workplace.md#함정-2--탈퇴-판정이-조용히-깨진다-️-이쪽이-중요)).
   `WorkService`의 탈퇴 검사가 `worker.user_id IS NULL`이었다.
   → `is_deleted`를 함께 본다. NULL 검사는 과거 하드 삭제로 생긴 행을 위해 유지.

4. **가명처리된 계정이 복구될 수 있었다.**
   `restoreUserByUserId`가 `is_deleted`만 0으로 되돌리면 이름도 소셜 연동도 없는
   유령 계정이 살아난다. → `anonymized_at IS NOT NULL`이면 거부.

### 접근 차단 범위 — 읽기 허용 · 쓰기 차단 (사용자 결정)

| | 동작 |
|---|---|
| 근무 등록 · 출퇴근 · 수정 · 삭제 | **차단** — 승인할 사장님이 없어 무의미 |
| 급여 설정 변경 | **차단** |
| 근무 · 급여 · 월별 정산 **조회** | **허용** — 보존의 목적이 알바생의 증빙이다 |
| 근무지 **탈퇴** | **허용** — 막으면 사라진 근무지에 영원히 묶인다 |

`hardDeleteUserById`는 저장소에서 아예 제거했다 — 되살리면 컴파일이 깨진다.

테스트 118개 통과. 뮤테이션 4건(자격증명 삭제 누락 · 배치 필터 제거 · 상태 판정 제거 ·
쓰기 게이트 무력화)이 모두 정확히 잡힌다.
...
### ✅ 10-2. 루틴 알람 — **서버 발송하지 않는다.** 로컬 알림으로 확정 (D3 답변)

#### 확인 결과 — 지금 아무도 알람을 못 받고 있다

서버: `alarm_time` 참조가 INSERT · ORDER BY · UPDATE · 조회 응답 채우기가 전부이고
**발송 경로가 없다.**

앱(`MOUP-iOS`): 로컬 알림을 **예약하는 코드가 0건**이다.

| 검색 | 결과 |
|---|---|
| `UNCalendarNotificationTrigger` | 0건 |
| `UNTimeIntervalNotificationTrigger` | 0건 |
| `UNMutableNotificationContent` | 0건 |
| `notificationCenter.add(...)` | 0건 |

`UNUserNotificationCenter`가 나오는 곳은 `App/AppDelegate.swift` 하나뿐이고 전부
**FCM 수신 설정**이다 — 권한 요청, 델리게이트 등록, `willPresent`/`didReceive` 핸들러.
보내는 쪽이 아니라 받는 쪽 코드다.

`alarmTime`은 DTO·UseCase·루틴 편집 화면에서만 쓰인다. 사용자가 시간을 고르면 서버로
보내고, 조회하면 화면에 다시 표시하는 것까지다. **그 시간에 무언가를 하는 코드가
양쪽 어디에도 없다.** 사용자는 알람 시간을 설정할 수 있지만 그 알람은 한 번도 울린 적이 없다.

> 개발 중 받아본 FCM 알림은 다른 경로다. 서버의 푸시 발송은 참가 요청·승인·거부·
> 관리자 개별 4곳이고 그건 정상 동작한다. 루틴 알람은 그중 어느 것도 아니다.

#### 결정 — 로컬 알림

**"분 단위 스캔이 무겁다"는 이유는 기각한다.** `@Scheduled(cron = "0 * * * * *")` +
인덱스 걸린 쿼리 하나면 하루 1440회다. 라즈베리파이 MySQL에서 이건 아무것도 아니다.
맞는 결론에 틀린 근거를 붙이면 다음 판단에서 같은 기준을 잘못 쓰게 된다.

**진짜 이유는 정시성이다.** FCM/APNs는 배달 시점을 보장하지 않는다. 기기가 잠깐
오프라인이면 재연결 후에 오고, iOS는 저전력·백그라운드에서 푸시를 지연시킬 수 있다.
**알람이 10분 늦게 오면 그건 알람이 아니다.** "오픈 준비 시작" 알림이 오픈 10분 뒤에
뜨면 없느니만 못하다. `UNCalendarNotificationTrigger`는 OS가 예약해두고 비행기 모드에서도
정시에 뜬다.

함께 사라지는 것들:
- FCM 의존이 하나 줄어든다 (키가 유출 상태라 재발급 전까지 그 경로 자체가 불안하다)
- 서버가 "오늘 이 루틴이 어느 근무의 것인지" 판정할 필요가 없다 — 앱은 그 화면을
  이미 그리고 있으므로 알고 있다. **제품 결정 하나가 통째로 사라진다.**
- 서버 다중화 시 중복 발송 방지도 불필요

비용: **iOS 대기 알림 64개 상한.** `repeats: true`로 (루틴, 요일) 단위로 걸면 개수가
아주 적어 문제없다. 근무마다 거는 방식이면 롤링 윈도우(예: 향후 30일)로 관리해야 한다.

> Android는 고려하지 않는다 — `MOUP-Android`는 테스트용 레포다.

#### 서버 작업

**없다.** `alarm_time` 저장·조회는 이미 동작한다. API 변경도 불필요하다.
남은 것은 **`MOUP-iOS`의 로컬 알림 예약 구현**이며, 이 원장의 범위 밖이다.
...
| ~~D3~~ | ~~루틴 알람 서버 발송~~ → **로컬 알림으로 확정, 서버 작업 없음.** 확인 결과 서버·앱 양쪽 다 저장만 하고 아무도 발송하지 않고 있었다 → [10-2](#-10-2-루틴-알람--서버-발송하지-않는다-로컬-알림으로-확정-d3-답변) |
...
## 배포 절차 — 근무지 카테고리 코드 전환

카테고리를 한글 표시명에서 영문 코드로 바꾸면서 **기존 데이터와 호환되지 않는 변경**이
생겼다. 신버전 서버는 `category_name`을 `WorkplaceCategory` 열거형으로 읽으므로,
한글이 남아 있는 DB를 조회하면 매핑 실패로 500이 난다. 조용히 틀리는 것보다 낫다고
판단해 호환 계층을 두지 않았다(실사용자 0명).

**DB 재초기화로 처리한다** (사용자 결정). 마이그레이션 스크립트도 만들었으나 이 경로에서는
쓰이지 않아 삭제했다 — 실행 검증을 못 한 SQL이 `db/migrations/`라는 권위 있어 보이는
자리에 남으면 나중에 누군가 그대로 믿고 운영 DB에 돌린다. 데이터를 보존해야 할 상황이
오면 커밋 `ef1e36a`에서 꺼내 쓸 수 있고, 매핑표는 `WorkplaceCategory` 주석에 남아 있다.

### 순서

1. **`develop` 푸시** — `deploy.yml`이 트리거되어 신버전 서버가 뜬다.
   이 시점부터 재초기화 전까지 기존 근무지 조회가 500이다. 실사용자가 없어 무해하다.
2. **`manual-db-init` 워크플로 실행** — 확인 문구 `DROP AND RECREATE` 입력.
   실행 직전 mysqldump 백업이 자동으로 뜬다.
3. 더미 데이터가 필요하면 `insert_dummy_data`를 체크한다(기본값 false).
   `moup_dummy_data.sql`의 카테고리는 이미 코드로 바꿔 뒀다.

> ⚠️ 2번 워크플로는 **이 브랜치(Phase 9 반영)에서 실행**해야 한다.
> `develop`의 구버전에는 확인 절차도 백업도 없고, `docker compose`에 `-f`가 빠져 있어
> 애초에 실행되지 않는다 — 저장소에 기본 `docker-compose.yml`이 없다(`ac42313`에서 수정).

### 검증

`GET /workplaces/{workplaceId}`의 `categoryName`이 `CVS` 같은 코드로 나오면 된다.
iOS 쪽에서는 근무지 수정 화면의 카테고리 프리필이 복구되고 "수정하기" 버튼이 활성화된다.
**영화관(`MOVIE_THEATER`) 등록·수정을 반드시 실측할 것** — 컬럼이 `VARCHAR(10)`이라
13자인 이 코드만 잘리던 경로였다(`ef1e36a`에서 `VARCHAR(20)`으로 수정).
...
| Phase | 상태 |
|---|---|
| 0 · 1 · 2 · 4 · 5 · 6 · 7 · 8 · 9 · 10-1 · 10-2 | ✅ **완료** |
| 3 | 🔶 3-1~3-7 완료. **3-8(근로소득세)만 남음** — 간이세액표 파일 + D6 대기 |
| 9 · Firebase 키 재발급 | 🔴 **미결 — 코드 아님. 사용자 작업** |

### 남은 것 둘

1. 🔴 **Firebase 서비스 계정 키 폐기·재발급.** 키가 public Docker 이미지에 들어 있다.
   시간이 지날수록 나빠지기만 한다. 코드로 해결할 수 없다.
2. **Phase 3-8** — 홈택스 근로소득 간이세액표 파일이 있어야 한다. 법정 표라 임의 생성
   불가. D6(공제대상 가족 수 — 1명 고정 권장)도 함께 답해야 한다.
3. ~~**`social_tokens` 평문 저장.**~~ → **암호화하지 않기로 결정.** 같은 덤프에 근무 기록·
   급여·주소가 이미 평문으로 있어 가장 쓸모없는 한 칸만 잠그는 셈이고, 실측한 OAuth
   스코프가 좁으며(Apple은 이름만), 키 일치 항목이 하나 늘면 실패가 조용하다.
   노출 창은 Phase 6에서 이미 줄였다 →
   [사유와 재검토 조건](scope-1-auth.md#i7-후속--소셜-토큰-암호화는-하지-않는다)

### 남은 제품 결정 — 코드 작업이 아니라 답이 필요한 것들

리뷰 findings 자체는 닫혔지만, **답을 못 받아 수정 방향을 정하지 못한 항목**이 남아 있다.
전부 "지금 동작이 틀렸다"가 아니라 "무엇이 맞는지 제품이 정해야 한다"에 가깝다.

| 출처 | 질문 | 걸린 것 |
|---|---|---|
| [3 Q2](scope-3-salary.md#확인-질문-8건) | 예정 종료시간이 있는데 일찍/늦게 퇴근하면 급여는 예정 기준인가 실제 기준인가. 확정 정책 1은 "실제 기록이 **없는**" 경우만 정의했다 | 3 I-1 |
| [3 Q3](scope-3-salary.md#확인-질문-8건) | 주휴수당 **개근 요건**(결근 시 미지급)을 반영할 것인가. 스키마에 결근 표현이 없다. 또 15시간 판정을 소정근로시간이 아니라 **실근로시간**으로 하고 있는데 의도된 근사인가 | 3 C-1 후속 |
| [3 Q4](scope-3-salary.md#확인-질문-8건) | 최저임금 하한을 서버가 강제할 것인가 (저장 거부 vs 경고). 매년 바뀌므로 배포 없이 갱신할 수단이 필요할 수 있다 | 3 I-5 |
| [3 Q6](scope-3-salary.md#확인-질문-8건) | 법정공휴일 가산(50%)이 로드맵에 있는가 | 3 I-11 |
| [3 Q7](scope-3-salary.md#확인-질문-8건) | 원 단위 **절사 vs 반올림**. 현재는 일관되게 절사(항상 근로자에게 불리)인데 명시적 정책인지 `(int)` 캐스팅의 부작용인지 불명확 | 3 M-1·M-2 |
| [3 Q8](scope-3-salary.md#확인-질문-8건) = [4 Q3](scope-4-user.md) | 사장님 홈 "인건비"가 알바생 **세후** 합계다. 사장님 부담은 세전 + 사업주 4대보험이 맞지 않는가 | 4 I10 |
| [4 Q1](scope-4-user.md) | 급여 설정 변경 시 **당월 이미 지난 근무일**의 `estimated_net_income`이 새 기준으로 갱신되는 것이 의도인가 | 확정 정책 2 해석 |

### ⚠️ 결정은 났는데 구현되지 않은 것 하나

[스코프 3 Q11](scope-3-salary.md#확인-질문-8건)은 **답변 완료**로 표시돼 있다 —
`SalaryUpdateRequest`에 `applyToFutureWorks` 플래그를 **기본 false**로 추가하고,
C-6이 만드는 범위 재계산 위에 얹는다는 결정이었다. 선행 조건 C-6은
[Phase 3-6](#-phase-3--3-13-7-완료-3-8세금만-남음)에서 끝났다.

**그런데 구현되지 않았고, 현재 동작은 결정과 반대다.** 코드베이스에 `applyToFutureWorks`가
0건이고, `WorkplaceService.updateWorkplace`의 알바생 분기는 급여를 수정할 때마다
**항상** 이번 달 이후 모든 월을 재계산한다. 즉 플래그 기본값 false(적용 안 함)와 정반대로
언제나 적용된다.

플래그를 넣을지, 아니면 "항상 적용"을 정식 동작으로 확정할지 정해야 한다.

### 이 원장의 범위를 벗어난 후속 작업

- **`MOUP-iOS` 로컬 알림 예약** ([10-2](#-10-2-루틴-알람--서버-발송하지-않는다-로컬-알림으로-확정-d3-답변)) —
  서버는 할 일이 없다.
- **`MOUP-iOS` 근무지 수정 폼의 더미 값** — `address: "기본 주소"`, `latitude: 0.0`을
  그대로 전송해 저장된 주소·좌표를 파괴한다. 서버는 [Phase 8 I-1](#-phase-8--완료-9de4ea6--545eb30--400cc16--ec114cc)에서
  부분 갱신으로 바꿔, 클라이언트가 필드를 **빼는** 것만으로 안전해지도록 해 뒀다.
